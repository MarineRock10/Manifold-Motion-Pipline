...
# sagittal crouch direction in IsaacLab joint order: hips flex, knees bend, ankles dorsiflex
CROUCH_DIRECTION = np.zeros(29)
CROUCH_DIRECTION[[0, 1]] = -0.6    # hip pitch L/R
CROUCH_DIRECTION[[9, 10]] = 1.0    # knee L/R
CROUCH_DIRECTION[[13, 14]] = -0.4  # ankle pitch L/R
...
class CrouchReference(PlannedReference):
    """Planner reference with a sagittal crouch offset applied to the leg joints.

    This is the first manifold-conditioned reference transform: the manifold's ceiling
    height maps to a scalar amount along `direction`, which lowers the whole-body pose
    while keeping the walking gait pattern generated by the planner.
    """

    def __init__(self, joint_pos, joint_vel, root_pos, root_quat, direction=None,
                 amount: float = 0.0, play: bool = True):
        super().__init__(joint_pos, joint_vel, root_pos, root_quat, play=play)
        self.direction = np.asarray(CROUCH_DIRECTION if direction is None else direction, dtype=np.float64)
        self.amount = float(amount)
        self._apply()

    @classmethod
    def from_plan(cls, planner, context_qpos, amount: float = 0.0, **plan_kwargs) -> "CrouchReference":
        raw = PlannedReference.from_plan(planner, context_qpos, **plan_kwargs)
        return cls(raw.joint_pos, raw.joint_vel, raw.root_pos, raw.root_quat, amount=amount)

    def set_amount(self, amount: float) -> None:
        delta = float(amount) - self.amount
        if abs(delta) > 1e-12:
            self.amount = float(amount)
            self.joint_pos = self.joint_pos + delta * self.direction[None, :]

    def _apply(self) -> None:
        if abs(self.amount) > 1e-12:
            self.joint_pos = self.joint_pos + self.amount * self.direction[None, :]

    def maybe_replan(self, planner, reserve: int = 16, context: np.ndarray | None = None,
                     force: bool = False, **plan_kwargs) -> bool:
        if not force and self.T - self.cursor > reserve:
            return False
        # splice raw-to-raw, then re-apply the crouch offset
        self.joint_pos = self.joint_pos - self.amount * self.direction[None, :]
        raw = PlannedReference.from_plan(planner, self.context_qpos() if context is None else context,
                                         **plan_kwargs)
        self.splice(raw)
        self._apply()
        return True
