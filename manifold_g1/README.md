...
- **Policy**: 10 Hz, action = body-frame velocity command `[vx, vy, wz]` scaled to
  ±1.5 m/s, ±0.6 m/s, ±1.0 rad/s. The command is mapped to the kinematic planner
  (`movement_direction` / `facing_direction` / `target_vel`), whose output is the
  motion reference tracked by frozen SONIC at 50 Hz.
- **Observation** (78-D): body-frame linear/angular velocity, gravity vector, joint
  positions relative to the default pose, joint velocities, goal vector in body frame,
  heading error, previous action, manifold parameters (L, W, H).
- **Reward**: progress toward the goal + goal bonus (10) − collision (capped
  penetration, 30/m) − energy (0.1) − fall penalty (10) − hard-collision penalty (5).
- **Termination**: fall (height < 0.45 m or tilt > 60°), success (< 0.35 m), hard
  collision, out of bounds; time limit 200 policy steps (20 s).
...
| L3 | 1x MuJoCo | height H swept | manifold height → body height | next |
