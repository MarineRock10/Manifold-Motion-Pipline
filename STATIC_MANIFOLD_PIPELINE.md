...
结论：这不是 bug，是**能力缺口**。但**"加大扰动"这条路已经试过，是错的**——两次微调实测：

| 训练时扰动 | r(sonic) 中位 | inside | 注释 |
|---|---|---|---|
| 无倾斜（当前发布的 policy） | **0.975** | **7/10** | 最好 |
| tilt 6°，30 轮 | 1.000 | 5/9 | 基本没变 |
| tilt 14°，20 轮 | 1.200 | 1/7 | **明显更差** |

tilt 14° 那次不仅倾斜行没救回来，**其余行也一起坏了**：waist 跟踪误差从 0.02–0.07 跳到
**0.26–0.31 rad**。原因清楚——加大倾斜后策略学会了靠 **waist pitch** 去凑，而 waist pitch
正是冻结控制器基本不跟的通道（见 §"控制器已知极限"）。所以它在做一个跟踪层执行不了的动作。

**教训**：倾斜这一维不能靠加大同一个扰动来解决；它要求的是一个跟踪层能执行的姿态通道，
而不是更多同方向的压力。这也是为什么"最大化包络内性"必须和"控制器能跟多少"一起看——
r(model) 在 tilt14 那次是 1.204，而 r(sonic) 1.164，模型和真机都在说同一件事：姿势本身错了。
