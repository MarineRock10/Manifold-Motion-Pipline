...
def run(args: argparse.Namespace) -> tuple[dict[str, Any], dict[str, np.ndarray]]:
    ...
    for online_iteration in range(args.online_condition_iterations):
        probe_data, probe_execution = execute_plan(args.scene, keyframes, dense_route, plan_options, args)
        ...
        if online_iteration + 1 < args.online_condition_iterations:
            ...
            continue
    data, execution = execute_plan(args.scene, keyframes, dense_route, plan_options, args)
    ...
    render_corridor = np.column_stack([
        ...
        np.repeat(envelope[None, :], len(world_route), axis=0), _route_yaw(world_route),
    ])
...
def main() -> int:
    ...
    if (args.num_candidates < 2 or args.online_condition_iterations < 0
            ...
            or min(args.goal_x, args.segment_length_m, args.fps,
                   ...
                   args.projection_handoff_weight, args.projection_corridor_weight,
                   args.projection_root_blend, args.projection_max_joint_step) <= 0):
