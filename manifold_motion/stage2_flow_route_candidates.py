...
class RouteFlowSampler:
    ...
    def __init__(self, windows: Path, autoencoder: Path, flow: Path, device: str = "cpu"):
        ...
        for primitive_id in PRIMITIVE_NAMES:
            path = C.REPO / "reports" / "manifold_motion" / f"stage2_mean_primitive{primitive_id}_v1" / "conditional_mean.pt"
            if path.is_file():
    ...
    def condition(self, primitive_id: int, corridor: np.ndarray, sdf: np.ndarray,
                  ...
                  history: np.ndarray | None = None) -> tuple[np.ndarray, int]:
        ...
        row = np.concatenate([
            self.normalizer.state(state_value).astype(np.float32),
            self.normalizer.state(history_value).reshape(1, -1).astype(np.float32),
            one_hot,
            self.normalizer.manifold(environment).astype(np.float32),
            self.normalizer.command(command[None]).astype(np.float32),
        ], axis=1)
        return row, index
    ...
    def sample(self, primitive_id: int, corridor: np.ndarray, sdf: np.ndarray,
               ...
               raw_exemplar: bool = False) -> tuple[np.ndarray, int]:
        ...
        if primitive_id in self.mean_models:
            with torch.no_grad():
                mean = self.mean_models[primitive_id](condition[:1]).cpu().numpy().reshape((1,) + self.data.target_shape)
            generated[0] = _target_from_model(
                self.normalizer.inverse_target(mean), self.data.joint_lower, self.data.joint_upper
            )[0]
        if raw_exemplar:
...
def execute_plan(scene: Path, keyframes: np.ndarray, dense_route: np.ndarray,
                 plan_options: list[list[CandidatePlan]], args: argparse.Namespace) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
    env = G1FlatEnv(scene)
    ...
    log: dict[str, list[Any]] = {key: [] for key in (
        ...
        "route_progress_m", "reference_phase", "body_route_yaw_error", "side_on_active",
    )}
    target_index = 1
    for tick in range(args.max_ticks):
        ...
        log["route_progress_m"].append(route_progress_m); log["reference_phase"].append(reference_phase)
